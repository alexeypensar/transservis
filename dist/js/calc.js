//calculator
'use strict'; // прайс - авто из Москвы в Якутск
...
var calcFrom = document.getElementById('calcFrom');
var calcTo = document.getElementById('calcTo');
var calcButton = document.getElementById('calcButton');
var calcResults = document.getElementById('calcResults');
var calcAutoPrice = document.getElementById('calcAutoPrice');
var calcAutoTime = document.getElementById('calcAutoTime');
var calcContainerPrice = document.getElementById('calcContainerPrice');
var calcWeight = document.getElementById('calcWeight');
var calcVolume = document.getElementById('calcVolume'); // калькулятор для автоперевозок:

var calcAuto = function calcAuto(type, weight, volume) {
  weight = +calcWeight.value;
  volume = +calcVolume.value;
  var factVolume = (volume * 1.1).toFixed(2);
  var basePrice;
  var resultPrice = 0; // по объёму:
...
var calcContainer = function calcContainer(type, weight, volume) {
  weight = +calcWeight.value;
  volume = +calcVolume.value;
  var factVolume = (volume * 1.1).toFixed(2);
  var basePrice;
  var resultPrice = 0;
  var factVolumeToWeight = weight / factVolume; // по объёму:
...
calcButton.addEventListener('click', function () {
  var calcTest = [calcFrom, calcTo, calcWeight, calcVolume];
  var calcTestErrors = 0;

  for (var i = 0; i < calcTest.length; i++) {
    if (calcTest[i].value === '') {
